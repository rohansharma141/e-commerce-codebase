...
- **A migration is subject to RLS.** It runs as the owning role, and FORCE applies policies to the owner, so a backfill with no tenant bound reads and writes nothing, without error. Orders' `0003` did that for three weeks behind a comment claiming the opposite. Lift RLS explicitly inside the migration's transaction, and verify a backfill against rows that exist *before* it runs — a cold database has none.
- **A pattern that registers is not a pattern that matches.** C-32b's guard was mounted on `storefront/(.*)`, which Express's path-to-regexp compiles to require a literal dot: it registered cleanly and guarded nothing, and a second layer returning the same error hid it. Prove a guard on every route it claims, using a request only that guard can refuse.
