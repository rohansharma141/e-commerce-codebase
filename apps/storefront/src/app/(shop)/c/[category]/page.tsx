...
import { channelHref, getChannelKey } from '@/lib/channel';
...
export default async function CategoryPage({ params, searchParams = {} }: CategoryPageProps) {
  ...
  const basePath = channelHref(`/c/${encodeURIComponent(category)}`);
  ...
  return (
    <main className="container mx-auto px-4 py-6">
      <nav className="mb-2 text-xs opacity-70">
        <a href={channelHref('/')} className="hover:opacity-100">Home</a>
      ...
      <div className="mb-6">
        <SearchBar money={money} basePath={basePath} channelKey={getChannelKey()} searchParams={searchParams} />
      </div>
