...
- **Never start Docker unaided.** Ask. Prefer `docker compose stop` over `down -v`; never `down -v` the user's running stack without asking. On this Windows machine `wsl --shutdown` reclaims the memory (measured 3.96 GB → 0; OpenSearch is ~89% of it).
