...
| Capabilities gain `channel`; the old tenant-level fields stay as deprecated aliases answering for the **default** channel, even when the request names another | Their meaning must not change under a consumer already reading them; ADR-0014 §7's expand/contract | C-18a |
| Channel edits invalidate the storefront's cached capabilities — any channel's, and only when something changed | Capabilities moved to channels as their source, so the invalidation had to move with them; a no-op write must not drop every page | C-18b |
| `taxRateBps` and `taxDisplay` stay tenant-level in capabilities | They must describe what is charged, and the money path does not yet charge a channel's own values (C-38, C-30) | C-18a |
