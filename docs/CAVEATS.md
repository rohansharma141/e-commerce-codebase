...
### Browse pages are not actually cached — the tags are correct but inert
- **Status:** open, and larger than it looks. Found while building H-3.
- **What:** the tag vocabulary is right and the api now names the affected categories, but nothing in the storefront is currently cached, so no `revalidateTag` call has an observable effect. Two signals agree: `next build` marks **every** route `ƒ (Dynamic) server-rendered on demand`, and two identical consecutive requests for `/c/laptop` each produce their own `search.completed` in the api log. A warm cache and a cold one look the same because there is no warm one.
- **Why:** every server read goes through `graphqlQuery`, which is a **POST**. Next.js's data cache stores GET responses; `next: { tags, revalidate }` on a POST is accepted and ignored. Tenant resolution reading `headers()` also opts every route into dynamic rendering, so there is no full-route cache to fall back on either.
- **Impact:** the storefront re-queries the api on every request. Correct, and slower than documented — the "regenerated by events" story describes an intent the running code does not implement. It also means the category-tag split shipped in H-3 cannot yet be demonstrated.
- **Fix:** make the reads cacheable before the tags can matter — either move the storefront's read path to `GET` with the query in the URL (which also gets HTTP caching for free), or wrap the fetch in `unstable_cache` keyed by document + variables + tenant, with the tenant read outside the cached callback. Then H-3's check becomes runnable.
- **How it got missed:** the tags were added, reviewed and documented without ever measuring whether a second request skipped the api. Nothing failed, because a cache that is never populated is also never wrong.
