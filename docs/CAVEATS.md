...
- **Status:** **half closed.** Decided 2026-09-22 (gate G-4 in [BACKLOG-channels.md](BACKLOG-channels.md)): refuse such a channel, then build per-channel price lists as their own phase. **C-32a** (2026-09-22) makes carts and checkout refuse it with `422 channel.unservable` — no cart, no order, no promotion use. **Reads are still served** in such a channel until C-32b; they show the price list's figures, which today's tenant-level capabilities label in the price list's currency, so nothing is mislabelled yet.
