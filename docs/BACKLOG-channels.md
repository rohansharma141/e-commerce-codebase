...
*Plan, worked out 2026-08-28 so it is not re-derived:* three deliverables in one commit — `docs/design/ADMIN-API.md` stating the five conventions; cursor pagination added to the four non-conforming list endpoints (`GET /admin/orders`, `/admin/prices`, `/admin/promotions`, `/admin/attribute-definitions`) with repository, controller and `@ApiProperty` response classes so `/docs-json` stays real; and `apps/api/src/admin-conventions.integration.spec.ts`, which boots the app in-process following the `checkout.integration.spec.ts` pattern and walks every admin list endpoint asserting cursor accepted, `{ items, nextCursor }` shape, page 2 not repeating page 1, and the error envelope on a 400 and a 404. Write the spec first, run it red against the unmigrated surface, then migrate. Two known risks: attribute-definition and product writes fire indexer handlers wanting OpenSearch — the bus isolates handler errors so writes still land, but if the spec needs it, gate on `TEST_OPENSEARCH_URL` the way the search suite does; and R-4's drift check means the OpenAPI regeneration must be committed in the same commit.
