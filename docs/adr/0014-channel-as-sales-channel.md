...
**Update, 2026-09-19 — this section's premise is not yet true in the code.** "A channel carrying one currency" assumes something charges it. Nothing does: `pricing.prices` holds one currency-less integer per product, and totals and checkout read currency from the tenant-level `pricing.tenant_config`, so an order placed in a EUR channel is charged in the tenant's GBP. The rule stated above — a missing price in a channel's currency must *fail*, never fall back to another currency's number — is the correct behaviour, and no code implements it yet. Together with §12's deferral of per-channel prices, that leaves two specified options: refuse to transact in a channel the price list cannot serve, or build per-channel price lists. That choice was gate G-4 in [BACKLOG-channels](../BACKLOG-channels.md). **Decided 2026-09-22: refuse now** (C-32 — carts, checkout and price reads in such a channel return a named error), **then build per-channel price lists** as their own phase with their own ADR.
